...
export const RegisterUser = async (req, res) => {
  try {
    ...
    if (verifyemail !== null) {
      ...
      res
        .status(200)
        .send({ status: true, user: user, message: "Đăng ký thành công" });
    }
  } catch (error) {
    res
      .status(503)
      .send({
        status: false,
        message: `Đăng ký không thành công lỗi ${error}`,
      });
  }
};
  ...
  try {
    ...
    if (user !== null) {
      ...
      if (verify) {
        ...
        res
          .status(503)
          .send({ status: false, message: `Sai tên đăng nhập hoặc mật khẩu` });
      }
    } else {
